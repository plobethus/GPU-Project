...
int main() {
    const char* filename = "5Matrix.txt"; //change for matrix file
    ...
    while (getline(infile, line, ',')) {
        ...
        int j = 0;
        while (j < 10 && (ss >> h_a[n][j])) ++j;
        ++n;
    }
    infile.close();

    // Host L and U
    float h_l[10][10] = {{0}}, h_u[10][10] = {{0}};
    ...
    // Copy back and print
    CUDA_CHECK(cudaMemcpy(h_l, d_l, bytes, cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(h_u, d_u, bytes, cudaMemcpyDeviceToHost));

    cout << "\nL Decomposition\n";
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j)
            printf("%10.6f ", h_l[i][j]);
        cout << "\n";
    }
